...
constexpr int RET_POP = 0x2FFFFFFF;
...
public:
	...
	int Visit(int value) {
		...
		if (ret != RET_BUSYTIMEOUT and ret != RET_TIMEOUT)
			int k = 0;
...
public:
	...
	int Pop()
	{
		while (true) {
			...
			if (ret != RET_BUSYTIMEOUT && ret != RET_TIMEOUT and ret != RET_POP) {
				if (ret == RET_POP)
					int k = 0;
				return ret;
			}
...
int main()
{
	...
	for (int n = 1; n <= MAX_THREADS; n = n * 2) {
		...
		my_stack.print20();
		std::cout << "교환 횟수: " << swap_count << std::endl;
